...
const defaultServer = process.env['ZEROXFILE_SERVER'] || 'https://zeroxfile.xyz'
const server = new Option('-s --server <url>', 'the 0xfile server to use').default(defaultServer)
...
program.name('0xfile').version(version).configureHelp({ showGlobalOptions: true })
